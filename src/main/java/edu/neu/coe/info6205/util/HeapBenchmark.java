...
public class HeapBenchmark {
    public static void main(String[] args) {
        for(int n=10000;n<=256000;n=n*2)
        {
            ...
            BaseHelper<Integer> helper = new InstrumentedHelper<>("HeapSortHelper1", config);
            HeapSort<Integer> heap = new HeapSort<>(helper);

            Consumer<Integer[]> consumer =RanArr -> heap.sort(RanArr);
            Benchmark_Timer<Integer[]> BT = new Benchmark_Timer<>("HeapSort Time Test " + n + " elements", consumer);
            int finalN = n;
        ...
        {
            System.out.println("=============HeapSort Time Test "+n+" elements"+"==========================");
            final Config config = Config.setupConfig("false", "0", "1", "", "");
            BaseHelper<Integer> helper = new InstrumentedHelper<>("HeapSortHelper1", config);
            HeapSort<Integer> heap = new HeapSort<>(helper);
            ...
            System.out.println("HeapSort Time Test "+n+" elements : " + randTime);
        }
    }
}
